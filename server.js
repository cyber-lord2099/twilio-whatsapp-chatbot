...
// Sub-schemes for each main scheme (updated for new main schemes)
const subSchemes = {
  '1': [
  ...
  '5': [], // मॅट्रिक पूर्व शिष्यवृत्ती योजना has no sub-schemes (already specific)
...
// Sub-scheme details (updated for new main schemes)
...
// Mapping of main scheme and sub-scheme numbers to image URLs (updated)
const schemeImages = {
  '1': {
    '1': null,
    '2': null,
...
// Helper to send WhatsApp text messages via Twilio with message splitting (unchanged)
...
// Helper to send WhatsApp image messages via Twilio (unchanged)
...
// List of main schemes (updated with new main schemes)
...
// List of sub-schemes for a selected main scheme (unchanged logic)
...
// Webhook to handle incoming messages from Twilio (updated for new main scheme count)
  ...
  if (!userState[from]) {
    ...
    if (/^\d+$/.test(choice)) {
      if (choice === '0') {
        userState[from] = { step: 'mainSchemes' };
        await sendMainSchemesList(from);
      } else if (choice >= 1 && choice <= 7) { // Updated to 7 main schemes
...
// Basic route for testing (unchanged)
app.get('/', (req, res) => {
